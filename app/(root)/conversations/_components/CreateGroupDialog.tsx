...
const createGroupFormSchema = z.object({
    name: z.string().min(1, {
        message: "This field ca't be empty"
    }),
    members: z.string().array().min(1, {
        message: "You must select at least 1 friend"
    })
})
...
const CreateGroupDialog = (props: Props) => {
    ...
    const { mutate: createGroup, pending } = useMutationState(api.conversation.createdGroup)
    ...
    const unselectedFriends = useMemo(() => {
        return friends ? friends.filter((friend: any) => !members.includes(friend._id)) : []
            ...
            <DialogContent className='block'>
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(handleSubmit)} className='space-y-8'>
                        ...
                        />
                        {
                            members && members.length ? (<Card className='flex items-center 
                            gap-3 overflow-x-auto w-full h-24 p-2 no-scrollbar'>
                                {
                                        ...
                                        return (
                                            <div key={friend._id} className="flex flex-col items-center gap-1">
                                                <div className="relative">
                                                    ...
                                                    <X
                                                        className="text-muted-foreground w-4 h-4 absolute bottom-8 left-7 bg-muted rounded-full"
                                                        onClick={() => {
